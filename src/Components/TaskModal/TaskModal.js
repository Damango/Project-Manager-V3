...
const TaskModal = (props) => {
	...
	function closeTaskModal() {
		let taskModalElement = document.querySelector(".task-modal-container");
		taskModalElement.style.transition = "0.3s";
		taskModalElement.style.opacity = "0";
		taskModalElement.style.right = "-1000px";
	...
	function findTaskIndex(taskID) {
		...
		let taskIndex = projectObject.tasks
			.map(function (e) {
				return e.taskID;
			})
			.indexOf(taskID);
	...
	function renderNewSubTaskInput() {
		if (addSubTaskInput) {
			return (
				<div className="new-sub-task-input-container">
			...
			<animated.div className="task-modal-container" style={modalAnimation}>
				...
				<div className="task-modal-category-changer-wrapper">
					<div className="task-modal-category-changer">
						<div
							className={"category-color-circle " + taskModalData.category}
						></div>
						<div className="category-changer-text">
							{taskModalData.category.toUpperCase()}
						</div>
					</div>
					<div
						className="category-changer-arrow-button"
				...
				<div className="task-modal-task-description task-block">
					...
					<div className="task-modal-task-description-text">
				...
				<div className="task-modal-subs-tasks task-block">
					...
					<div className="task-modal-sub-tasks-container">
						{taskModalData.subTasks.map((task, index) => (
							<SubTask
